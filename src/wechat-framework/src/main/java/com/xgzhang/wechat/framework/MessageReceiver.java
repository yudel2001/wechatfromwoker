...
public class MessageReceiver {
	...
	public void receiveGetMesseage(HttpServletRequest request, HttpServletResponse response){ 
		...
		try{
			_logger.info("testGet aaaaaaaaaaaaaaaa--------------------------------------");
	...
	public static void main(String[] args){
		 String bigStr ="wechat_token"+ "1465204839" +"199463361 503 0.005 0.005 288";
		String signature="af511e54b7e090517ea98ce175dd54a9e8630c40";
		System.out.println(SecurityUtility.getDigestOfString(bigStr.getBytes()).toLowerCase());
		System.out.println(signature.equals(SecurityUtility.getDigestOfString(bigStr.getBytes()).toLowerCase()));
	}
	public void receivePostMessage(HttpServletRequest request, HttpServletResponse response){
		...
		try{
			...
			this._logger.debug("-----------&&&&&&&&&&&-------------");
			this._logger.debug(String.format("MessageType = %s", receiveEntity.getMsgType()));
			this._logger.debug(String.format("TextContent = %s", receiveEntity.getContent()));
			this._logger.debug(String.format("PicRUL = %s", receiveEntity.getPicUrl()));
			this._logger.debug(String.format("EventType = %s", receiveEntity.getEventKey()));
